...
> **Release channel:** this site tracks `main` and currently targets the `0.1.0-alpha.18` repository candidate. Verify public availability with npm dist-tags. Alpha.18 derives `llms.txt` freshness from content `updatedAt`; the guarded release workflow reruns the complete gate before publication.
