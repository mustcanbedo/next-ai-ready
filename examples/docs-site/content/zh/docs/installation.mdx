...
> **发布渠道：** 本站跟随 `main`，npm 的 `latest` 与 `alpha` 当前均为 `0.1.0-alpha.16`。2026-08-02 已从公共 registry 通过 `pnpm + Next.js 16` 与 `npm + Next.js 15` 的干净安装、CLI 流程和生产构建；生产 MCP 也已通过带认证 initialize 与三个页面发现工具调用。
