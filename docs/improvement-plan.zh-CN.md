...
| Next.js 兼容验证 | `进行中` | 旧矩阵使用 `next.config.mjs`，未覆盖当前 `create-next-app` 生成的 `next.config.ts`；2026-08-02 真实接入发现 CJS 导出与 `NextConfig` 类型兼容缺口，修复完成后需重跑六组合 |
...
| G1-03 | 固化真实安装矩阵 | `进行中` | 已将外部 smoke 改为在 Next.js 15/16 使用真实 `next.config.ts`；修复 CJS 配置入口与官方 `NextConfig` 类型兼容后，需重跑 tarball 与 registry 六组合 |
...
- 2026-08-02 重新执行仓库外 `create-next-app` 接入时，`doctor` 达到 100/100，但正式 `next build` 暴露两个此前矩阵未覆盖的缺口：`next-ai-ready/config` 缙少 CommonJS 入口，以及 `withAiReady()` 的类型约束不兼容官方 `NextConfig`。当前分支已增加专用 CJS 配置产物、放宽包装器泛型并让外部 smoke 在 Next.js 15/16 使用 `next.config.ts`；单组 pnpm × Next.js 15 tarball 回归已通过。
