...
> **发布渠道：** 本站跟随 `main`，npm 当前为 `0.1.0-alpha.12`。alpha.12 存在已知的 TypeScript Action 模块加载问题，已在 `main` 修复并会随下一个预发布版本发布。以下导入示例使用已发布 alpha 的 API。
...
`main` 已提供 `next-ai-ready/actions`、`next-ai-ready/config`、`next-ai-ready/json-ld`、`next-ai-ready/robots` 与 `next-ai-ready/audit` 专用入口，但 npm alpha.12 尚未包含；下一个预发布版本上线前，它们会明确标为仅限 `main`。
