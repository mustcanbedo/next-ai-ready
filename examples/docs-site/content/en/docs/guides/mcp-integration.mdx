...
| `search_pages` | `query`, `locale?`, `limit?` | Deterministic local lexical search with CJK-aware tokenization. Queries are 1–200 characters; locale can narrow multilingual graphs; the default result limit is 5 and the maximum is 20. |
...
## One search implementation across protocols

`search_pages` and the docs site's HTTP `search_docs` action both use the public `PageSearchProvider` contract. The built-in `createGraphSearchProvider(graph)` keeps retrieval deterministic and database-free. A runtime index can implement the same contract and be injected into `registerAiReady(server, { graph, searchProvider })` without changing MCP clients.
