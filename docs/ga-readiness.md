...
**Current development branch version:** `0.1.0-alpha.15`. **npm `@alpha`:** `0.1.0-alpha.15` published. Audit v3, focused runtime exports, TypeScript `next.config.ts` compatibility, and MCP page discovery are in the public package. The full public-registry compatibility matrix and authenticated production MCP path passed on 2026-08-02; see [improvement-plan.zh-CN.md](./improvement-plan.zh-CN.md) for final **`0.1.0`** GA sign-off.
...
| 2a | **Restore GitHub Actions** `NPM_TOKEN` | Maintainer | Repository secret was empty during the alpha.15 run; workflow now fails before the release gate with a clear error. Add a valid npm granular token, then rerun the idempotent alpha workflow |
...
- [x] Current `create-next-app` → `pnpm add next-ai-ready@alpha` → `init` → `build` → `doctor` → `next build`
...
- [x] npm **`@alpha` = alpha.15** published (2026-08-02)
