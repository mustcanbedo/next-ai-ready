  ...
  "evidence": [
    {
      ...
      "observation": "认证门已存在，但协议验证需要部署凭据。"
    }
  ],
  "review": "recommended"
}
...
`status` 继续表示 CI 与计分严重性，`judgment` 则记录现有证据究竟能证明什么。
网络失败会成为低置信度 `unknown` 并要求复核；受凭据限制的检查在完成授权部署
验证之前也保持未知，而不会被误报为确定通过或失败。
