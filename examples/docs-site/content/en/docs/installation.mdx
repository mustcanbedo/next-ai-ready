...
> **Release channel:** this site tracks `main`; npm `latest` and `alpha` currently serve `0.1.0-alpha.16`. On August 2, 2026, the published `latest` passed clean public-registry `pnpm + Next.js 16` and `npm + Next.js 15` installs, the CLI workflow, and production builds. The authenticated production MCP endpoint also passed initialize and all three page discovery calls.
