...
|--------|----------------|---------|
| `next-ai-ready` | No (authoring + CLI) | `init`, `build`, `doctor`, `defineConfig`, `defineAction` |
| `next-ai-ready/config` | Loaded by Next config | `withAiReady()` without build-time scanner imports |
| `next-ai-ready/robots` | Yes | Dynamic `app/robots.ts` policies |
| `next-ai-ready/hooks` | Yes | Runtime observability |
| `next-ai-ready/handlers/*` | Yes (`server-only`) | Route stubs in `app/%5Fai-ready/` |
| `next-ai-ready/audit` | No | Programmatic deployment audits |
...
`init` also patches `next.config` with `withAiReady()` from `next-ai-ready/config` and adds `next-ai-ready build` to your build script when missing.
...
`next-ai-ready build` emits `public/robots.txt` with explicit AI-bot rules unless you set `emit: { robots: false }`. For dynamic policies, add `app/robots.ts` and use `aiRobots()` from `next-ai-ready/robots` — Next.js serves `app/robots.ts` at runtime. Doctor treats that as valid and does not require a static `public/robots.txt`.
