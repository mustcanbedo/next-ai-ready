...
可重渲染源文件位于 [`video/`](./video/)，composition 为 `NextAiReady-ZH-Vertical`。

| 时间 | 画面 | 旁白/字幕 |
| --- | --- | --- |
| 0-5 秒 | 原网页、`/llms.txt`、逐页 Markdown 和 MCP 四个结果同时出现 | 不改 Next.js 页面，给 AI 一套干净入口。 |
| 5-9 秒 | 接入前访问 `/llms.txt` 返回 404 | 没有发现入口和稳定正文时，检索只能先猜页面。 |
| 9-18 秒 | 终端运行 generator、安装和 `init` | 安装一个公开包，不改变现有 App Router 页面。 |
| 18-25 秒 | `build` 依次生成 `llms.txt`、Markdown 与 `sitemap.md` | 机器可读产物由同一份内容确定性生成。 |
| 25-31 秒 | MCP `search_pages` 命中安装页，再由 `get_page` 读取 | Agent 可以按语言找到并读取正确页面。 |
| 31-37 秒 | 生产文档站第三方可读性分数与三条边界 | 100/100 只代表技术可读性，不保证收录、排名或引用。 |
| 37-45 秒 | 一条创建命令、中文教程和 GitHub 仓库 | MIT 开源，欢迎拿真实项目来测试并反馈阻塞。 |
