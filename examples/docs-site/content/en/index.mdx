...
This documentation site tracks `main` and currently targets the `0.1.0-alpha.20` repository candidate. Public availability is determined by npm dist-tags, not by the repository version. Alpha.20 adds one replaceable page-search provider shared by MCP and HTTP, a bilingual search-intent evaluation gate, purpose-based AI crawler policies, and traceable Audit v3 judgments.
