...
> **发布渠道：** 本站跟随 `main`，当前目标为仓库候选版本 `0.1.0-alpha.19`。公开可用性以 npm dist-tags 为准。alpha.19 包含 alpha.18 基于内容 `updatedAt` 的 `llms.txt` 新鲜度改动，并补齐 MCP 到 LLMS 的公开依赖链；受保护发布工作流会在发布前重新运行完整门禁。
