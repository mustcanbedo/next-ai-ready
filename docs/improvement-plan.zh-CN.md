...
| G1-08 | 恢复 GitHub Actions npm 发布身份 | `进行中` | workflow 已增加发布门禁前的 Token 非空检查与 `npm whoami`；仍需写入有效的仓库 Secret `NPM_TOKEN`，再幂等重跑 alpha workflow |
