...
| `search_pages` | `query`、`locale?`、`limit?` | 支持中文分词的确定性本地词法搜索。查询长度为 1–200 字符，可按 locale 缩小多语言结果，默认返回 5 条，最多 20 条。 |

搜索只读取预构建的 SemanticGraph，不需要外部 API、Embedding 服务或向量数据库。将 `list_pages` 或 `search_pages` 返回的路由传给 `get_page` 即可读取全文。

## 多协议共用一套检索实现

`search_pages` 与文档站的 HTTP `search_docs` Action 都使用公开的 `PageSearchProvider` 契约。内置 `createGraphSearchProvider(graph)` 保持确定性且无需数据库；运行时索引只需实现同一契约，再通过 `registerAiReady(server, { graph, searchProvider })` 注入，无需修改 MCP 客户端。
