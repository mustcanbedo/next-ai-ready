...
- `public/llms-full.txt` — full content dump.
...
1. **Config** — `ai-ready.config.mjs` exists with required `site.name`, `site.baseUrl`, and `site.description`.
2. **Actions** — loads actions, counts public vs private, warns if public actions lack `whenToUse`.
3. **Build artifacts** — warns if `.next-ai-ready/graph.json` is missing (run `build` first).
4. **Route stubs** — warns if `app/_ai-ready/llms-txt/route.ts` or `app/_ai-ready/md/[...path]/route.ts` are missing.
