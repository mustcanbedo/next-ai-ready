...
Every v3 check contains a bounded `judgment` object:
  ...
  "evidence": [
    {
      ...
      "sourceUrl": "https://example.com/api/mcp/mcp",
      "observation": "The authentication gate is present; protocol verification needs deployment credentials."
    }
  ],
  "review": "recommended"
}
...
`status` remains the CI and scoring severity. `judgment` records what the evidence
establishes. Network failures become low-confidence `unknown` results with required
review, while credential-bound checks remain unknown until an authorized deployment
verification runs.
