...
> **发布渠道：** 本站跟随 `main`，当前目标为仓库候选版本 `0.1.0-alpha.20`。公开可用性以 npm dist-tags 为准。alpha.20 增加 MCP 与 HTTP 共用的可替换页面搜索、双语检索评测、按用途划分的 AI 爬虫策略及可追溯的 Audit v3 判断；受保护发布工作流会在发布前重新运行完整门禁。
