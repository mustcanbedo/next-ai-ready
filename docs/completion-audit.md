...
>
> **当前发布补充（2026-08-02）：** npm `latest` 与 `alpha` 均为 `0.1.0-alpha.17`；Audit v3、MCP 页面发现与 locale 过滤、运行时专用入口、TypeScript `next.config.ts` 兼容修复、npm 可发现性元数据及 Nextra/Fumadocs 内容发现均已发布。alpha.15 的公共 registry 六组合矩阵、alpha.16 的代表性 `latest` 复验、alpha.17 的完整发布门禁、manifest 核验与公共 registry Next.js 16 构建及生产 MCP 带认证调用均已通过；当前优先级见 [`improvement-plan.zh-CN.md`](./improvement-plan.zh-CN.md)。下方正文保留 alpha.11 审查快照。
...
- [x] npm `latest` 与 `alpha` → **0.1.0-alpha.17**（2026-08-02；完整发布门禁及 npm manifest 核验通过；alpha.16 代表性复验与 alpha.15 六组合矩阵保留）
