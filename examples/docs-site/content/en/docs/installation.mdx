...
> **Release channel:** this site tracks `main` and currently targets the `0.1.0-alpha.20` repository candidate. Verify public availability with npm dist-tags. Alpha.20 adds shared, replaceable MCP and HTTP page search, bilingual retrieval evaluation, purpose-based AI crawler policies, and traceable Audit v3 judgments; the guarded release workflow reruns the complete gate before publication.
