...
| 导入 | 需要 Next.js？ | 用途 |
|------|----------------|------|
| `next-ai-ready` | 否（编写期 + CLI） | `init`、`build`、`doctor`、`defineConfig`、`defineAction` |
| `next-ai-ready/config` | 由 Next 配置加载 | 不引入构建期扫描器的 `withAiReady()` |
| `next-ai-ready/robots` | 是 | 动态 `app/robots.ts` 策略 |
| `next-ai-ready/hooks` | 是 | 运行时观测 |
| `next-ai-ready/handlers/*` | 是（`server-only`） | `app/%5Fai-ready/` 路由 stub |
| `next-ai-ready/audit` | 否 | 以代码方式执行部署审计 |
...
`init` 还会从 `next-ai-ready/config` 为 `next.config` 注入 `withAiReady()`，并在缺失时将 `next-ai-ready build` 加入 build 脚本。
...
`next-ai-ready build` 默认生成 `public/robots.txt`；若设置 `emit: { robots: false }` 则跳过。动态策略请在 `app/robots.ts` 中使用来自 `next-ai-ready/robots` 的 `aiRobots()`。Doctor 认可该组合，不会误报缺少静态 `public/robots.txt`。
