...
This documentation site tracks `main` and currently targets the `0.1.0-alpha.19` repository candidate. Public availability is determined by npm dist-tags, not by the repository version. Alpha.19 carries the alpha.18 deterministic `llms.txt` freshness change and closes the public MCP-to-LLMS dependency graph.
