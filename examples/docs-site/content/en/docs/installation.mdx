...
> **Release channel:** this site tracks `main`; npm currently serves `0.1.0-alpha.12`. Alpha.12 has a known TypeScript Action-module loading issue that is fixed on `main` and will ship in the next prerelease. The import examples below use the published alpha surface.
...
Focused `next-ai-ready/actions`, `next-ai-ready/config`, `next-ai-ready/json-ld`, `next-ai-ready/robots`, and `next-ai-ready/audit` exports exist on `main` but are not present in npm alpha.12. They are documented as main-only until the next prerelease.
