...
> **说明（docs-site）：** 本站采用**双轨**架构。浏览器 UI 通过 `MdxContent` 渲染简化 Markdown；AI 构建管线使用完整的 `@next-ai-ready/mdx` 编译器。请为 AI 轨编写 frontmatter（`summary`、`questions`、`tags`），即使 UI 轨不执行 MDX export。
...
export const semantic = {
  ...
  questions: [
    { q: "如何安装 next-ai-ready？", a: "运行 `pnpm add next-ai-ready@alpha`。" },
...
运行 `pnpm add next-ai-ready@alpha`。
