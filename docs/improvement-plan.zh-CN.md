...
> 当前仓库候选：`next-ai-ready@0.1.0-alpha.19`（公开 npm 状态以 dist-tags 为准）
> 当前主分支基线：`ac7c5ea`（PR #27 已合并，合并后 CI 全部通过）
> 待发布变更：确定性 `llms.txt` 新鲜度与完整发布依赖链；候选包为 `next-ai-ready@alpha.19`、`@next-ai-ready/next@alpha.19`、`@next-ai-ready/mcp@alpha.16` 和 `@next-ai-ready/llms@alpha.16`
...
| npm GA | `进行中` | 仓库候选为 `0.1.0-alpha.19`；Release Alpha 在发布前验证完整门禁、manifest 与 npm 标签，公开状态不得从 `main` 推断；尚未退出 prerelease 或发布 `0.1.0` |
...
| G1-07 | 消除 `main`、npm 与网站文档的发布漂移 | `进行中` | alpha.19 候选已同步 SDK、README、双语文档站与机器可读产物；发布工作流仍需核对 registry 和 dist-tags，且不得在发布前写成已发布 |
...
| ID | 任务 | 状态 | 验收标准 |
|---|---|---|---|
| J2-01 | 按用途配置 AI 爬虫策略 | `待验证` | 搜索、训练、用户触发和未知用途可独立配置；旧字符串 API 向后兼容；`doctor` 不惩罚单纯的训练退出 |
| J2-02 | 为语义审计定义有限判断结果 | `待商榷` | 每条规则先定义 `pass/fail/unknown`、证据和置信度；不得只返回不可追溯的总分 |
| J2-03 | 建立搜索意图与页面匹配评估集 | `待商榷` | 使用真实查询、目标页面和人工标注验证判断一致性，先证明能改善内容决策再进入 SDK |
